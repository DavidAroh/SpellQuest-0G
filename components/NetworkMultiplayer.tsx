...
const HAND_SMOOTH_MIN = 0.3;
const HAND_SMOOTH_MAX = 0.85;
...
export const NetworkMultiplayer: React.FC = () => {
  ...
  // True while a modal is open — freezes hand input (timer keeps running so the match stays fair)
  ...
  useEffect(() => { pausedRef.current = showSettings || showHowTo; }, [showSettings, showHowTo]);
  ...
  // Sound engine — reads the latest mute state via ref
  ...
  const hostNextWord = useCallback((activeConn?: DataConnection) => {
    ...
    if (!c) return;
    const nextWordText = pickRandomWord(categoryRef.current, difficultyRef.current);
    c.send({ type: "START_WORD", word: nextWordText, difficulty: difficultyRef.current, category: categoryRef.current });
    // Apply locally
  ...
  const setupConnListeners = useCallback((conn: DataConnection) => {
    conn.on("data", (data: any) => {
      ...
      if (data.type === "START_WORD") {
        ...
        // Adopt the host's difficulty & category so scoring and the badge match
        // the host's word (refs set immediately so the score multiplier is correct).
        if (data.difficulty) { setDifficulty(data.difficulty); difficultyRef.current = data.difficulty; }
        if (data.category) { setCategory(data.category); categoryRef.current = data.category; }
  ...
  // Tear down the current session and return to the lobby (used after a
  // mid-game disconnect so players are never stranded on a dead game screen).
    ...
    const onResults = (results: any) => {
      ...
      if (!isCorrectRef.current || tiles.current.some(t => t.inTray)) {
        ...
        if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
          ...
          // Amplify reach so the bottom rows are easy to grab without dropping the hand out of frame.
          ...
          // Skeleton drawn at the amplified cursor coordinates so it stays glued
          // to the grab cursor (not the raw, washed-out video hand).
        ...
        if (handPos && isPinching && !isCorrectRef.current && !pausedRef.current) {
          if (!activeTileId.current) {
            ...
            if (tile) { tile.x += (handPos.x - tile.x) * DRAG_FOLLOW; tile.y += (handPos.y - tile.y) * DRAG_FOLLOW; }
          }
  ...
  if (gamePhase === "LOBBY") {
    return (
      <div className="flex flex-col items-center justify-center w-full min-h-screen relative overflow-hidden"
        ...
        {showHowTo && <HowToPlay mode="network" onClose={() => setShowHowTo(false)} />}

        {/* How-to-play button */}
        <button onClick={() => setShowHowTo(true)}
          className="absolute top-5 left-5 z-20 flex items-center gap-2 bg-white/10 hover:bg-white/20 border border-white/20 text-white/80 px-4 py-2 rounded-full text-sm font-bold transition-all active:scale-95">
    ...
    <div className="flex w-full h-[100dvh] bg-gradient-to-br from-blue-50 to-purple-50 overflow-hidden font-roboto relative">
              ...
              {isHost ? (
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-2xl px-4 py-3 text-center">
                  <p className="text-xs font-bold text-blue-700 uppercase tracking-wide mb-1">
                    {DIFFICULTY_SETTINGS[difficulty].label} • {category}
                  </p>
                  <p className="text-xs text-gray-500">The host controls difficulty &amp; category.</p>
                </div>
              )}
              <div>
      ...
      <div ref={containerRef} className="flex-1 relative h-full overflow-hidden">
        ...
        <div className="absolute top-6 left-6 z-40 flex items-center gap-4">
          ...
          <button onClick={() => setShowHowTo(true)} title="How to play"
        ...
        {/* Success celebration — centered card over a soft backdrop so it never
            collides with the word card or the tiles. Live spelling shows in the tray. */}
        {isCorrect && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-[2px] animate-in fade-in duration-300">
            <div className="bg-white rounded-[36px] shadow-2xl border-4 border-green-300 px-10 py-8 text-center max-w-sm mx-6 animate-in zoom-in-95 duration-300">
              ...
              {isHost ? (
                ...
                <div className="w-full bg-gray-100 text-gray-500 px-6 py-4 rounded-2xl text-sm font-bold">
        ...
        {/* Mid-game disconnect overlay — keeps players from being stranded */}
        {connectionStatus === "error" && (
          <div className="absolute inset-0 flex items-center justify-center z-[55]"
            style={{ background: "linear-gradient(135deg, rgba(15,12,41,0.97), rgba(48,43,99,0.97), rgba(36,36,62,0.97))" }}>
            <div className="flex flex-col items-center gap-6 text-center px-6">
              <div className="bg-red-500/20 border border-red-500/40 p-5 rounded-full">
                <WifiOff className="w-10 h-10 text-red-400" />
              </div>
              <div>
                <p className="text-2xl font-black text-white mb-2">Connection Lost</p>
                <p className="text-white/60 font-semibold">{errorMsg || "The match has ended."}</p>
              </div>
              <button
                onClick={returnToLobby}
                className="bg-gradient-to-r from-purple-500 to-purple-700 hover:from-purple-400 hover:to-purple-600 text-white font-black px-8 py-4 rounded-2xl transition-all hover:scale-105 active:scale-95 shadow-lg shadow-purple-500/30 text-lg"
              >
                Back to Lobby
              </button>
            </div>
